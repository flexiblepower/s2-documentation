...
Since the pairing process depends on the deployment of the endpoints, an endpoint that is deployed in a LAN must be able to determine its peer endpoint is deployed in the same LAN or in the WAN. From the perspective of a LAN deployed endpoint, a remote endpoint **must** be considered to be in the same LAN only if the pairing request originates from a link-local address. If the address is not link-local, the endpoint shall be considered to be deployed in the WAN.
...
| --- | --- | --- |
| If the server endpoint is deployed in the LAN, and the client endpoint claims to be deployed in the LAN, does the request originate from with in the LAN (see [LAN deployment check](#lan-deployment-check))? | `Other` | No |
...
| If the client endpoint is deployed in the LAN, and the server endpoint claims to be deployed in the LAN, does the response originate from within the LAN (see [LAN deployment check](#lan-deployment-check))? | call `/finalizePairing` where `success` is `false` |
...
| When provided, did the value of `role` in the `clientNodeDescription` change compared to the stored value? | `CommunicationDetailsErrorMessage` with errorMessage `Other` | Retry later |
| When provided, did the value of `deployment` in the `clientEndpointDescription` change compared to the stored value? | `CommunicationDetailsErrorMessage` with errorMessage `Other` | Retry later |
...
| When provided, did the value of `role` in the `serverNodeDescription` change compared to the stored value? | Do not proceed and try again later with step 1 |
| When provided, did the value of `deployment` in the `serverEndpointDescription` change compared to the stored value? | Do not proceed and try again later with step 1 |
