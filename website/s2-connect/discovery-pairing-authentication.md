...
| Communication client | The nodes which behaves as the HTTP client when initiating an S2 session. |
| Communication server | The nodes which behaves as the HTTP server when initiating an S2 session. |
...
| Pairing client | The endpoint which behaves as the HTTP client when pairing with a node. |
...
| Pairing server | The endpoint which behaves as the HTTP server when pairing with a node. |
...
| `serverNodeId` | The node ID of the node at the server that the end user intents to pair wth the node at the client |
