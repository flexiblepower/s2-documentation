...
The following sequence diagram is an example of what a message exchange between the CEM and RM could look like, but messages could also be sent in a different order (see also [State of communication](State_of_communication) and the [FRBC Message reference](FRBC_messages)). `ReceptionStatus` messages are omitted for readability.
