...
The following sequence diagram is an example of what a message exchange between a CEM and RM could look like, but messages could also be sent in a different order (see also [State of communication](https://github.com/flexiblepower/s2-ws-json/wiki/State_of_communication) and the [PEBC Message reference](https://github.com/flexiblepower/s2-ws-json/wiki/PEBC_messages)). ReceptionStatus messages are omitted for readability.
